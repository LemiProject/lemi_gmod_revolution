...
void draw_model_execute_hook::hook(draw_model_state_t& draw_state, model_render_info_t& render_info, void* bone)
{
	...
	if (!settings::states["visuals::chams_enabled"] || !interfaces::engine->is_in_game() || render_system::vars::is_screen_grab || interfaces::engine->is_taking_screenshot() || (settings::other::anti_obs && settings::visuals::chams_obs_check))
